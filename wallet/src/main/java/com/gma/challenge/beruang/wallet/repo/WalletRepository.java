package com.gma.challenge.beruang.wallet.repo;
...
public interface WalletRepository extends JpaRepository<Wallet, Long> {
    ...
    @Query(
            value =
                    "select * from wallet where wallet.id in (select w_c.wallet_id from"
                            + " wallet_categories w_c where w_c.categories_id in :categoryIds)",
            nativeQuery = true)
